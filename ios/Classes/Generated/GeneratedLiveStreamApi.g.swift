...
protocol LiveStreamHostApi {
  ...
  func getSensorOrientation(cameraId: String) throws -> Int64
  func isPreviewPreTransformed() throws -> Bool
}
...
class LiveStreamHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: LiveStreamHostApi?, messageChannelSuffix: String = "") {
    ...
    let getSensorOrientationChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.apivideo_live_stream.LiveStreamHostApi.getSensorOrientation\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getSensorOrientationChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let cameraIdArg = args[0] as! String
        do {
          let result = try api.getSensorOrientation(cameraId: cameraIdArg)
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      getSensorOrientationChannel.setMessageHandler(nil)
    }
    let isPreviewPreTransformedChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.apivideo_live_stream.LiveStreamHostApi.isPreviewPreTransformed\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      isPreviewPreTransformedChannel.setMessageHandler { _, reply in
        do {
          let result = try api.isPreviewPreTransformed()
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      isPreviewPreTransformedChannel.setMessageHandler(nil)
    }
  }
}
...
/// Generated protocol from Pigeon that represents a handler of messages from Flutter.
protocol DeviceOrientationManagerHostApi {
  func getUiOrientation() throws -> String
}

/// Generated setup class from Pigeon to handle messages through the `binaryMessenger`.
class DeviceOrientationManagerHostApiSetup {
  static var codec: FlutterStandardMessageCodec { GeneratedLiveStreamApiPigeonCodec.shared }
  /// Sets up an instance of `DeviceOrientationManagerHostApi` to handle messages through the `binaryMessenger`.
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: DeviceOrientationManagerHostApi?, messageChannelSuffix: String = "") {
    let channelSuffix = messageChannelSuffix.count > 0 ? ".\(messageChannelSuffix)" : ""
    let getUiOrientationChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.apivideo_live_stream.DeviceOrientationManagerHostApi.getUiOrientation\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      getUiOrientationChannel.setMessageHandler { _, reply in
        do {
          let result = try api.getUiOrientation()
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
      ...
      getUiOrientationChannel.setMessageHandler(nil)
    }
  }
}
